...
function checkState()
    ...
    if compareBlock("forward", "minecraft:spruce_sapling") then
        state = "idle"
    elseif not blockExists("up") and not blockExists("down") then
        state = "descending"
    elseif compareBlock("up", "minecraft:spruce_log") or compareBlock("up", "minecraft:spruce_leaves") then
        state = "chopping"
    else
        state = "searching"
    end
...
function idle()
    ...
    while waiting do
        sleep(5)
        if not compareBlock("forward", "minecraft:spruce_sapling") then
            waiting = false
        end
    end
end
...
function search()
    if compareBlock("forward", "minecraft:spruce_log") then 
        turtle.dig()
        turtle.forward()
    else
        turtle.forward()
        if compareBlock("up", "minecraft:spruce_log") then
            return
        elseif compareBlock("forward", "minecraft:spruce_log") then
            turtle.dig()
            turtle.forward()
        else
            turtle.turnLeft()
...
function returnToIdlePos()
    while true do
        ...
        elseif inspectDown.name == "minecraft:podzol" then
            turtle.forward()
        else
            turtle.back()
            turtle.turnLeft()
    ...
    if turtle.getFuelLevel() < threshold then
        for i=1, 16 do
            if turtle.getItemDetail(i) ~= nil then
                if turtle.getItemDetail(i).name == "minecraft:spruce_log" then
                    turtle.select(i)
                    turtle.refuel()
                end
            end
